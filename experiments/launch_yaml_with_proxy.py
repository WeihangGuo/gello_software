"""
Enhanced GELLO launch script with optional Talos proxy support.

This is an upgraded version of launch_yaml.py that adds proxy support while
maintaining backward compatibility with all existing YAML configs.

Usage:
    # Without proxy (same as launch_yaml.py)
    python launch_yaml_with_proxy.py --left-config-path configs/spacemouse_ur_sim.yaml
    
    # With proxy (add proxy section to YAML)
    python launch_yaml_with_proxy.py --left-config-path configs/spacemouse_ur_sim_with_proxy.yaml
"""
...
def main():
    ...
    if proxy_enabled:
        print("\n" + "=" * 60)
        print("🛡️  TALOS PROXY MODE ENABLED")
        print("=" * 60)
        proxy_port = proxy_cfg.get("port", 6001)
        robot_port = cfg["robot"].get("port", 6002)
        print(f"Robot server will bind to: {robot_port}")
        print(f"Client will connect to proxy: {proxy_port}")
        print("=" * 60 + "\n")
    # ========================================
    ...
    if hasattr(robot, "serve"):  # MujocoRobotServer or ZMQServerRobot
        ...
        # ========== PROXY SUPPORT (NEW) ==========
        # If proxy enabled, client connects to proxy port instead of server port
        if proxy_enabled:
            client_port = proxy_cfg.get("port", 6001)
            client_host = proxy_cfg.get("host", "127.0.0.1")
            print(f"🔗 Connecting to Talos proxy at {client_host}:{client_port}...")
        else:
            # Original behavior: client connects directly to server
            client_port = server_port
            client_host = server_host
        
        # Create client to communicate with server (or proxy if enabled)
        robot_client = ZMQClientRobot(port=client_port, host=client_host)
        # ========================================
        ...
        # ========== PROXY SUPPORT (NEW) ==========
        # If proxy enabled, client connects to proxy port
        if proxy_enabled:
            client_port = proxy_cfg.get("port", 6001)
            client_host = proxy_cfg.get("host", "127.0.0.1")
            print(f"🔗 Connecting to Talos proxy at {client_host}:{client_port}...")
            ...
            # Original behavior: client connects directly to hardware
    ...
    # ========== PROXY SUPPORT (NEW) ==========
    if proxy_enabled:
        print("\n⚠️  Commands will flow through Talos safety proxy")
        print(f"Architecture: Agent → Client (:{client_port}) → Talos Proxy → Robot Server (:{server_port})\n")
    # ========================================
